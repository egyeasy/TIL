...
def apart(k, n):
    if k == 0:
        ...
        return sum([apart(k-1, i) for i in range(1, n+1)])
...
# 생각
# 1. 이중 for문 돌릴 때 초기화나 업데이트의 위치가 중요하다. 문제의 숫자를 잘 파악해서 인덱싱도 신경써야 함.
# 2. 재귀로 짜면 코드가 간단해서 좋지만 시간 초과가 뜬다.